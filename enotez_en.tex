...
\begin{beschreibung}
 ...
 \Option{list-style}{<style>}\Default{plain}
   ...
   details.
 \Option{list-preamble-skip}\Default{\cmd{medskipamount}}\sinceversion{0.5}%
   Sets the vertical skip that is inserted if a list preamble is inserted by
   using either \cmd{AtNextEndnotesList} or \cmd{AtEveryEndnotesList}. It's
   default is set equal to \cmd{medskipamount}.
 \Option{list-postamble-skip}\Default{\cmd{medskipamount}}\sinceversion{0.5}%
   Sets the vertical skip that is inserted if a list postamble is inserted by
   using either \cmd{AfterNextEndnotesList} or \cmd{AfterEveryEndnotesList}.
